...
public class HideSubCommand extends BrigadierCommand {
    ...
    public @NotNull Component descriptionComponent() {
        return StringUtil.color("<primary>/ie hide <secondary><flag></secondary>").hoverEvent(StringUtil.color("""
                <primary>Adds an item flag to your item.
                \s
                ◼ <secondary><flag><required></secondary> The item flag to add.
                ◼ <secondary>[-all]<optional></secondary> Add all item flags from the item.
                ◼ <secondary>[-view]<optional></secondary> View all your item's current flags.""").asHoverEvent()).clickEvent(ClickEvent.suggestCommand("/ie hide "));
    }
                ...
                .then(Commands.argument("flag", GenericEnumArgument.generic(ItemFlag.class))
                        .executes(context -> updateItemFlags(context, meta -> {
                            ...
                            if (meta.hasItemFlag(flag)) {
                                Utils.tell(context.getSource(), "<red>Your item already has the item flag: <yellow>" + flag.name());
                                return null;
                            }

                            meta.addItemFlags(flag);
                            Utils.tell(context.getSource(), "<primary>Added <secondary>" + flag.name().toLowerCase().replace("_", " ") + "</secondary> item flag to your item.");
                            return meta;
                        }))
                )
                ;
